...
import { Copy, DownloadIcon, LinkIcon } from "lucide-react"
...
export const ImageItem = ({ image }: Props) => {
    ...
    return (
        <div className="bg-gray-800/40 backdrop-blur-md shadow-lg rounded-2xl overflow-hidden flex flex-col transition-all duration-300 hover:shadow-xl hover:bg-gray-800/60">
            ...
            {/* Imagem com overlay */}
            <div className="relative w-full h-72 md:h-64 lg:h-72 overflow-hidden group">
                <AppImage
                    width={400}
                    height={400}
                    src={image.url}
            ...
            <div className="p-4 flex flex-col gap-3">
                ...
                {image.author && (
                    <span className="text-xs text-gray-500 dark:text-gray-400">
                        Postado por {image.author.display_name || image.author.name}
                    </span>
                )}
            </div>
        </div>
